...
function JUEGO() {
    ...
    function perdiste() {
        if ((document.getElementById("Meteiorito").offsetLeft > 630) ||
            ...
            // Aumentamos el contador de derrotas
            derrotasNivel1 += 1;
            // Comprobamos si ha perdido 2 veces
...
// Función para volver de las Reglas al Menú Principal (inversa a Mover())
function VolverAlMenu() {
    // 1. Obtenemos las secciones que vamos a mover
    var seccionReglas = document.getElementById("Reglas");
    var seccionMenu = document.getElementById("Seccion_01");

    // 2. Movemos la sección de Reglas hacia abajo, fuera de la pantalla
    seccionReglas.style.top = "120%";
    seccionReglas.style.transition = "1s";

    // 3. Después de un breve momento, hacemos que el menú principal vuelva a aparecer
    //    y lo movemos a su posición original.
    setTimeout(function () {
        seccionMenu.style.display = "flex"; // Nos aseguramos que sea visible
        seccionMenu.style.top = "0";
        seccionMenu.style.transition = "1s";
    }, 400); // Un pequeño retraso para que la animación sea fluida
}
...
//cerrar la pista
document.getElementById("Contenedor_Pista").addEventListener('click', function () {
    document.getElementById("Contenedor_Pista").style.display = "none";
});